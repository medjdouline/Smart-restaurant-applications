...
class MenuBoissonPage extends StatefulWidget {
  const MenuBoissonPage({Key? key}) : super(key: key);

  @override
  State<MenuBoissonPage> createState() => _MenuBoissonPageState();
}

class _MenuBoissonPageState extends State<MenuBoissonPage> {
  Item? _selectedBoisson;
  double _currentRating = 0;

  void _showBoissonDetails(Item boisson) {
    setState(() {
  ...
  void _closeDetails() {
    setState(() => _selectedBoisson = null);
  }
...
Future<void> _refreshFavorites() async {
      ...
      body: Container(
        ...
        child: Stack(
          children: [
            ...
            if (_selectedBoisson != null) 
              _buildBoissonDetailCard(favorisService, cartService, ratingService),
          ],
        ),
      ),
    );
  ...
  Widget _buildContent(MenuService menuService, FavorisService favorisService) {
      ...
      return const Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 16),
            Text('Chargement des boissons...', style: TextStyle(color: Colors.brown, fontSize: 16)),
          ],
        ),
      );
    }
    ...
    final groupedBoissons = menuService.groupedBoissons;

    if (groupedBoissons.isEmpty) {
      return const Center(
        child: Text('Aucune boisson disponible', style: TextStyle(color: Colors.brown, fontSize: 18)),
      );
    }
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          for (var subCategory in groupedBoissons.keys)
            Column(
                  ...
                  child: SizedBox(
                    ...
                    child: ListView(
                      ...
                      children: [
                        ...
                        for (var boisson in groupedBoissons[subCategory]!)
                          Padding(
                            padding: const EdgeInsets.symmetric(horizontal: 8),
                            child: _buildBoissonCard(boisson, favorisService),
                          ),
                        SizedBox(width: MediaQuery.of(context).size.width * 0.05),
                      ],
                    ),
  ...
  Widget _buildBoissonCard(Item boisson, FavorisService favorisService) {
    return GestureDetector(
      onTap: () => _showBoissonDetails(boisson),
      child: Container(
        width: 150,
        decoration: BoxDecoration(
          borderRadius: BorderRadius.circular(12),
          image: DecorationImage(
            image: AssetImage(boisson.image),
            fit: BoxFit.cover,
          ),
          boxShadow: [
            BoxShadow(
        ...
        child: Stack(
          children: [
            ...
            Positioned(
              ...
              child: IconButton(
                icon: Icon(
                  _isFavorite(favorisService, boisson.id) ? Icons.favorite : Icons.favorite_border,
                  color: _isFavorite(favorisService, boisson.id) ? Colors.red : Colors.white,
                  size: 24,
                ),
                onPressed: () => _toggleFavorite(favorisService, boisson),
              ),
            ),
            Positioned(
              bottom: 10,
  ...
  Widget _buildBoissonDetailCard(FavorisService favorisService, CartService cartService, RatingService ratingService) {
    final averageRating = ratingService.getAverageRating(_selectedBoisson!.id);
    final ratingCount = ratingService.getRatingCount(_selectedBoisson!.id);
                      ...
                      Positioned(
                        ...
                        child: Row(
                          children: [
                            IconButton(
                              icon: Icon(
                                _isFavorite(favorisService, _selectedBoisson!.id)
                                    ? Icons.favorite
                                    : Icons.favorite_border,
                                color: _isFavorite(favorisService, _selectedBoisson!.id) ? Colors.red : Colors.white,
                              ),
                              onPressed: () => _toggleFavorite(favorisService, _selectedBoisson!),
                            ),
                    ...
                    child: Column(
                      children: [
                        ...
                        Padding(
                          ...
                          child: Text(_selectedBoisson!.description, textAlign: TextAlign.center, style: const TextStyle(fontSize: 16)),
                        ),
                        const SizedBox(height: 20),
                        ...
                        Padding(
                          padding: const EdgeInsets.symmetric(vertical: 8),
                          child: Text(_selectedBoisson!.ingredients, textAlign: TextAlign.center, style: const TextStyle(fontSize: 16)),
                        ),
                        const SizedBox(height: 20),
                        ...
                        ElevatedButton(
                          ...
                          onPressed: () {
                            if (_currentRating > 0) {
                              ratingService.addRating(_selectedBoisson!.id, _currentRating);
